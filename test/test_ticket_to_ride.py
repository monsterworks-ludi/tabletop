...
def test_ttr_simple():
    ...
    # Fig 4.10a, p. 75
    assert south_cost_matrix.equals(sp.Matrix([[x**2, 2 * x**2], [0, 2 * x**2]]))
    ...
    # Example, p. 76
    assert north_points_matrix.equals(sp.Matrix([[v**2, 0], [v**4, 2 * v**2]]))
    # Example, p. 76
    ...
    assert south_combined_matrix.equals(
        sp.Matrix([[x**2 * v**2, 2 * x**2 * v**2], [0, 2 * x**2 * v**2]])
    )
